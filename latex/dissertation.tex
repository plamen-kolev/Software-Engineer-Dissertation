...
\subsection{Security}
{\color{red} 
	Vagrant ssh insecure keys, pem certificate authentication, packages from official sources, no password storage, cleaning certificates after download.
}

Virtual machine instance isolation, privacy and authorisation is one of the most important security concerns. This section describes the decisions and considerations made during the development process.

{\color{red} Hashing}
{\color{red} SSH Credentials without password}
{\color{red} Potential for adding own keys}
{\color{red} PS AUX}
{\color{red} guest isolation benefits/drawbacks}
...
For the purpose of this work, the decision to have limited scope of allowed Linux distributions to Ubuntu, Centos and debian, but it is important to note that the architecture allows extending that list and development was done with that in mind.
