...
class MachineNewTest < Minitest::Test
  ...
  def test_create_valid_vm_centos
    vm_user = 'centosuser'
    vm = Deeploy::VM.new(
      owner: @user,
      distribution: 'centos',
    ...
    find_in_file(puppet_path, vm_user, "Expecting to find name #{vm_user} in file #{puppet_path}")
    find_in_file(puppet_path, '/usr/bin/yum check-update -y', "Expecting to find line yum update in file #{puppet_path}, but was unable")
  ...
  def test_create_valid_vm_ubuntu
    ...
    find_in_file(puppet_path, vm_user, "Expecting to find name #{vm_user} in file #{puppet_path}")
    find_in_file(puppet_path, 'apt-get update', "Expecting to find line apt-get update in file #{puppet_path}, but was unable")
    # check puppet syntax error
    puppet_validation_command = "puppet parser validate #{puppet_path}"
  ...
  def find_in_file(file, line, exception)
    line_found = false
